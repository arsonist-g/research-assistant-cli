...
验证目标：headless Edge（DEC-028：`--headless=new` + 本机真实 UA）+ `cfbypass.solve` 能真实
通过 CF 挑战并取回内容。走 fetch 的 CF 兜底完整路径 `_fetch_with_stealth`（含系统代理解析 +
solve + html2md），即用户实际用 `fetch <cf-url>` 时的链路。solve 失败会抛 AntibotError
（测试据此失败并报详情）。**不弹浏览器窗口**（headless）。
依赖：本地 Edge/Chromium 可执行、外网（含系统代理）可达。
"""
...
from research_assistant.fetch import browser
...
async def test_fetch_returns_content_behind_cloudflare(cf_url: str):
    ...
    即代表 headless + solve 真实通过了 CF；拿不到则 pytest 直接展示 AntibotError 详情。
    """
    _skip_if_unusable()
