...
写带 frontmatter 的 markdown：默认 tmp-doc/<YYYY-MM-DD>/scrape-<slug>-<HH-MM-SS>-<rand>.md，或 --write 指定路径。
...
def write_snapshot(url: str, method: str, md: str, write_path: str | None) -> str:
    ...
    否则 → tmp-doc/<YYYY-MM-DD>/scrape-<slug>-<HH-MM-SS>-<rand>.md，同名已存在则追加序号，绝不裸覆盖。
    """
    ...
    slug = _slugify(url)
    now = datetime.now()
    # 按日期分二级目录（年-月-日）；ts 仅保留时分秒，日期已在目录层，不再重复
    date_dir = now.strftime("%Y-%m-%d")
    ts = now.strftime("%H-%M-%S")
    rand = secrets.token_hex(3)
    tmp_dir = Path("tmp-doc") / date_dir
    tmp_dir.mkdir(parents=True, exist_ok=True)
    path = _unique_path(tmp_dir / f"scrape-{slug}-{ts}-{rand}.md")
